...
def _handle_movement_rotation(player, key_press):
    move_actions = {
        "up": lambda: (
            player.pos[0] > 0 and not (player.rot == 3 and player.pos[0] == 1),
            -1,
            0,
            player.board.move_down,
        ),
        "down": lambda: (
            player.pos[0] < 4 and not (player.rot == 1 and player.pos[0] == 3),
            1,
            0,
            player.board.move_up,
        ),
        "right": lambda: (
            player.pos[1] < 4 and not (player.rot == 0 and player.pos[1] == 3),
            0,
            1,
            player.board.move_left,
        ),
        "left": lambda: (
            player.pos[1] > 0 and not (player.rot == 2 and player.pos[1] == 1),
            0,
            -1,
...
def create_players(
    screen: pygame.Surface, brick_size: int, nr_of_players: int
) -> list[Player]:

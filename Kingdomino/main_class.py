...
class game:
    ...
    def _setup_screen(self,title='Game'):
        ...
        brick_selection_position = (screen_size[0] / 2 - config.CHOOSE_BRICK_OFFSET_X, config.CHOOSE_BRICK_OFFSET_Y)
        pygame.display.set_caption(title)
        return screen, brick_selection_position

    def _create_players_names(self):
        player_queue = Actions.create_players(self.screen, config.BRICK_SIZE, config.number_of_players)
    ...
    def run_game(self):
        ...
        for round in range(nr_of_rounds):
            self.pile.draw_player_bricks(self.player_queue, self.screen, (self.brick_selection_position[0], self.brick_selection_position[1] + 100 + config.BRICK_SIZE * 4), config.BRICK_SIZE)
    ...
    def _first_round(self):
        ...
        self.player_queue,self.running = Actions.choose_bricks(self.player_queue, brick4, self.brick_selection_position, self.screen, self.pile, config.BRICK_SIZE)

    def pick_new_bricks(self,round,max_rounds):
        if round != max_rounds-1:
            brick4 = self.pile.get4()
            self.pile.take4(brick4, self.screen, self.brick_selection_position, config.BRICK_SIZE)
            pygame.display.flip()
            self.player_queue,self.running = Actions.choose_bricks(self.player_queue, brick4, self.brick_selection_position, self.screen, self.pile, config.BRICK_SIZE)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key in action_map:
                    ask = Actions.place_brick(player, action_map[event.key], self.screen)
